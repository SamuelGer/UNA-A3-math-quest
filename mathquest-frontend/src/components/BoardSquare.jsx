...
export default function BoardSquare({ numero, tipo, jogadores }) {
  ...
  return (
    <div
      ...
      <span style={{fontFamily:'var(--font-display)',fontSize: bp === 'compact'? 7 : 9,
          color:'rgba(255,255,255,0.55)',lineHeight:1}}>{numero}</span>
      ...
      {temAlguem && (
        <div style={{position:'absolute',top:-18,left:'50%',transform:'translateX(-50%)',display:'flex',gap:1,zIndex:10}}>
          {aqui.map((j,i) => (
            <span key={i} style={{fontSize: bp === 'compact' ? 12 : 16,
                filter:'drop-shadow(0 2px 5px rgba(0,0,0,0.9))',animation:'pawnBounce 1.1s infinite ease-in-out',animationDelay:`${i*0.15}s`}}>{j.emoji}</span>
          ))}
        </div>
      )}
