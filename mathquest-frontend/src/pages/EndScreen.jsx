  ...
  return (
    <div style={{position:'relative',width:'100vw',height:'100vh',display:'flex',alignItems:'center',justifyContent:'center',background:'radial-gradient(ellipse at center,#1A1408 0%,#0A0A06 70%)',overflow:'hidden'}}>
      ...
      <motion.div initial={{opacity:0,scale:0.88}} animate={{opacity:1,scale:1}} transition={{duration:0.55}}
        style={{position:'relative',zIndex:1,textAlign:'center',padding:'40px 36px',background:'rgba(14,14,8,0.97)',border:'1px solid var(--gold-dim)',borderRadius:18,width:'90%',maxWidth:480,boxShadow:'0 0 80px rgba(212,160,23,0.14)',display:'flex',flexDirection:'column',alignItems:'center',gap:16}}>

        <motion.div animate={{y:[0,-12,0]}} transition={{duration:2,repeat:Infinity,ease:'easeInOut'}}
          style={{fontSize:64,filter:'drop-shadow(0 0 18px rgba(212,160,23,0.55))'}}>🏆</motion.div>

        <div>
          <h1 style={{fontFamily:'var(--font-display)',fontSize:30,fontWeight:900,color:'var(--gold)',letterSpacing:3,textShadow:'0 0 28px rgba(212,160,23,0.4)'}}>Parabéns!</h1>
        ...
        <div style={{width:'100%',display:'flex',flexDirection:'column',gap:8}}>
          ...
          {ranking.map((j, i) => (
            <motion.div key={i} initial={{opacity:0,x:-20}} animate={{opacity:1,x:0}} transition={{delay:i*0.1}}
              ...
              <span style={{fontFamily:'var(--font-display)',fontSize:16,fontWeight:900,color: i===0?'var(--gold)':'#8B7D5A'}}>{j.pontos} pts</span>
            </motion.div>
          ))}
        </div>
